...
INPUT_FILE  = "../Datasets/data/raw_reviews_improved.csv"
OUTPUT_FILE = "../Datasets/data/cleaned_reviews_improved.csv"
AUDIT_FILE  = "../Datasets/data/cleaning_audit.csv"

MIN_WORDS    = 3
MIN_CHARS    = 8
SHORT_GENERIC = {
    "bagus", "jelek", "mantap", "buruk", "lumayan", "keren",
    "parah", "oke", "ok", "good", "bad",
}

# Rating 3 dibuang karena dianggap ulasan ambigu (hanya butuh positif dan negatif).
# negatif (rating 1-2) vs positif (rating 4-5)
EXCLUDED_RATINGS = {3}
...
def run_cleaning():
    ...
    print(f"Total raw reviews: {len(df_reviews):,}")
    print(f"Distribusi rating (sebelum filter):\n{df_reviews['rating'].value_counts().sort_index()}\n")

    # ── FIX: Buang rating netral (3) sejak awal ──────────────
    before = len(df_reviews)
    df_reviews = df_reviews[~df_reviews["rating"].isin(EXCLUDED_RATINGS)].copy()
    after = len(df_reviews)
    print(f"Rating {EXCLUDED_RATINGS} dibuang: {before - after:,} baris dihapus")
    print(f"Sisa setelah filter rating: {after:,}")
    print(f"Distribusi rating (sesudah filter):\n{df_reviews['rating'].value_counts().sort_index()}\n")
    ...
    df_reviews["text_dedup_key"] = (
        df_reviews["app_id"].astype(str) + " || " + df_reviews["text"]
    )
    ...
    df_reviews.loc[df_reviews["short_generic_flag"],      "keep"] = False
    df_reviews.loc[df_reviews["repeat_ratio"] > 0.65,    "keep"] = False

    audit_cols = [
        "app_name", "rating", "text_raw", "text",
        "char_len", "word_len", "short_generic_flag",
        "repeat_ratio", "content_ok", "keep",
    ]
    df_reviews[audit_cols].to_csv(AUDIT_FILE, index=False, encoding="utf-8-sig")

    df_clean = df_reviews[df_reviews["keep"] == True].copy()
    keep_cols = [
        "app_id", "app_name", "review_id", "text_raw", "text",
        "rating", "thumbs_up", "review_created",
        "reply_content", "replied_at", "sort_source",
    ]
